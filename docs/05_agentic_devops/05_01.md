...
```plaintext
I need to create a new GitHub Actions workflow to build a Docker image from the code in the src/ folder, push it to an Azure Container Registry, and then deploy it to an Azure Container App. The Dockerfile is located in the src/ folder and I need to include only the files in that folder for the Docker image. The workflow should be triggered on a push to the main branch. I also need to copy the contents of a GitHub secret named ENV to a new .env file inside the container, but I must NOT check in my .env file or make any secrets public. After pushing the image, the workflow should update the Container App to use the new image.
```
...
3. Build and push the Docker container image to Azure Container Registry, with an image name of `chat-app`.
4. Deploy the new image to the Azure Container App by updating the container revision.
