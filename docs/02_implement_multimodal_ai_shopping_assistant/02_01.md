...
After that, retrieve the necessary environment variables for your Azure OpenAI deployment. For this, you will use the `gpt-5.4-mini` deployment that you created in the prior exercise. Add the following code to retrieve these values:
...
The next step is to create an AzureOpenAI client using managed identity authentication. Add the following code to create the client:
...
Open the `src/chat_app.py` file. Then, go to **line 247** and comment out the following line:

```python
await websocket.send_text(fast_json_dumps({"answer": "This application is not yet ready to serve results. Please check back later.", "agent": None, "cart": persistent_cart}))
```

After commenting this out, uncomment **line 254**, which contains the following code:

```python
await handle_single_agent(websocket, user_message, persistent_cart)
```

This calls the `handle_single_agent()` function in `src/handlers/single_agent_handler.py`, which in turn calls `generate_response()` from `singleAgentExample.py`. The `generate_response()` function will route the user message through to your gpt-5.4-mini deployment.

Then, uncomment **line 45** to import the handler:
