...
# Task 02 - Create a Red Teaming Agent via code
...
The evaluation that you ran in the first section used a simple test function as the target model. In this task, you will configure the AI Red Teaming Agent to scan an actual deployed Azure OpenAI model. The `OpenAIChatTarget` class from the PyRIT library provides a convenient way to define an OpenAI chat model as the target for red teaming.

<details markdown="block">
<summary><strong>Expand this section to view the solution</strong></summary>

First, remove the `test_chat_target()` function from the `redTeamingAgent_initializer.py` file. Then, add the following code to your script, replacing what you removed, to define the Azure OpenAI chat target model:
...
There is no convenient helper in the PyRIT library that allows us to call a Microsoft Foundry agent for our red teaming evaluation. Therefore, we will define a custom target that makes an HTTP request against the Cora agent.
...
This will execute the red team scan against the Cora agent. After the scan is complete, review the results in the JSON file as you did for the prior example.
