...
Next, retrieve the names of your Azure AI Search and Cosmos DB resources. You can do this by reviewing the output of the Bicep deployment in the Azure portal, or by navigating to the resource group in the portal and looking for the resources there. You will also need the resource group name, which is `techworkshop-l300-ai-agents` unless you named it differently. This script will also retrieve the signed-in user's principal ID.
...
Once you have these variables set, run the following commands. This will first enable the system-assigned managed identity for the Azure AI Search service, then retrieve its principal ID. Finally, it will run commands to grant your signed-in user the `Cosmos DB Built-in Data Contributor` for your Azure account, and `Cosmos DB Account Reader Role` for the Azure AI Search managed identity created by the Bicep script:
...
This will give your signed-in user the necessary permissions to write to Cosmos DB, as well as giving the Azure AI Search system-assigned managed identity appropriate access to read from a Cosmos DB container.
