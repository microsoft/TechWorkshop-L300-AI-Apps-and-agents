...
Next, you will need to retrieve your signed-in user's principal ID and the principal ID of the Azure AI Search managed identity created by the Bicep script. You will also need to know the resource group name and the Cosmos DB account name. You can set these values as variables in your terminal session using the following commands:

```bash
resourceGroup="techworkshop-l300-ai-agents"
cosmosDbAccountName="{YOUR_COSMOS_DB_ACCOUNT_NAME}"
principalId=`az ad signed-in-user show --query id -o tsv`
aiSearchManagedIdentityId=`az search service show --resource-group $resourceGroup --name "{YOUR_AZURE_AI_SEARCH_NAME}" --query identity.principalId -o tsv`
```

If your resource group is named differently, make sure to update the `resourceGroup` variable accordingly.

You can retrieve the name of your Cosmos DB account by navigating to the resource group in the [Azure portal](https://portal.azure.com/) and looking for the Cosmos DB account resource. The name will be listed there.

Then, run the following commands to grant your signed-in user the `Cosmos DB Built-in Data Contributor` for your Azure account, and `Cosmos DB Account Reader Role` for the Azure AI Search managed identity created by the Bicep script:
...
az role assignment create --assignee $principalId --role "Cosmos DB Account Reader Role" --scope "/subscriptions/$(az account show --query id -o tsv)/resourceGroups/$resourceGroup/providers/Microsoft.DocumentDB/databaseAccounts/$cosmosDbAccountName"
```
