        ...
        while True:
            ...
            try:
                ...
                product_intent_keywords = [
                    "paint", "color", "product", "products", "recommend", "buy", "purchase",
                    "available", "availability", "inventory", "looking for", "need", "price"
                ]
                is_cora_product_intent = (
                    agent_name == "cora"
                    and any(keyword in user_message.lower() for keyword in product_intent_keywords)
                )

                if agent_name in ["interior_designer", "interior_designer_create_image"] or is_cora_product_intent:
                    ...
                    try:
                        ...
                        products = await asyncio.wait_for(
                            loop.run_in_executor(thread_pool, product_recommendations, search_query),
                            timeout=8.0
                        )
                    except asyncio.TimeoutError:
                        logger.warning("Product recommendations timed out", extra={"agent": agent_name})
                        products = []
                    except Exception as product_error:
                        logger.warning("Product recommendations failed", extra={"agent": agent_name, "error": str(product_error)})
                        products = []
