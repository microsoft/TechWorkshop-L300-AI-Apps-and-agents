...
On **line 68**, you will see a call to `configure_azure_monitor()`. This call enables telemetry collection and is why we see the information that we do in the **Tracing** menu. Immediately after this line (that is, on **line 69**), uncomment the following line of code to instrument OpenAI operations.

```python
OpenAIInstrumentor().instrument()
```

Repeat this process in two more files: `src/app/agents/agent_processor.py` and `src/app/tools/discountLogic.py`. For `agent_processor.py`, uncomment **lines 31 and 36**. For `discountLogic.py`, uncomment **lines 12 and 17**.
...
Push the Docker image to your Azure Container Registry by running the following command:

```bash
docker push {YOUR_REGISTRY_NAME}.azurecr.io/chat-app:latest
```

Finally, update the Container App to deploy the new image revision:
...
It may take a minute for the Container App to pull the new image and start the revision. Once the application is running, you can navigate to the FQDN of your Container App to access the chat application.
