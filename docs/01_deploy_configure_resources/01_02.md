...
![Collect information for gpt-5.4-mini deployment.](../../media/Solution/0102_FoundryEndpoints3.png)

- `gpt_endpoint`: Copy the **Target URI** box from the **gpt-5.4-mini** deployment and save everything prior to `/openai` in the URI. It should look something like `https://{YOUR_AI_FOUNDRY}.cognitiveservices.azure.com/`.
...
{: .note }
> The .env file also includes values for an MCP server location and agent IDs. You will use these during Exercise 2 and do not need to modify them.

#### Production: Set Container App environment variables
...
After deploying your models in the previous step, you need to set the four model-specific endpoint variables on the Container App. Run the following command, replacing the placeholder values with the endpoints you collected above:
...
Alternatively, you can set these in the [Azure portal](https://portal.azure.com/) by navigating to your Container App, selecting **Containers** from the **Application** menu, and then selecting **Edit and deploy**. In the container settings, select the **Environment variables** tab to add or update values.
