...
Ensure that **Agent** is the active radio button and select **Next** to move to the next step.
...
{: .note }
> Ensure that you have selected **Agent** from the evaluation list, not **Dataset**. If you do not see `sample.output_text` in the drop-down to fill in for Response, this is the most likely reason.
