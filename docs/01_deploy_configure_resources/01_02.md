...
- `gpt_endpoint`: Copy the **Target URI** box from the **gpt-5.4-mini** deployment and save everything prior to `/api` in the URI. It should look something like `https://{YOUR_AI_FOUNDRY}.services.ai.azure.com`. If the Target URI looks like your Foundry endpoint, please use `https://{YOUR_AI_FOUNDRY}.services.ai.azure.com` instead.
