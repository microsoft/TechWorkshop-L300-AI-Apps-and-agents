...
{: .note }
> This training uses **managed identities** (via `DefaultAzureCredential`) to authenticate to all Azure services. You do not need to copy any API keys or connection strings. The `.env` file only contains endpoints, deployment names, and non-secret configuration.
